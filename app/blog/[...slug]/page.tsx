...
import { GET_BLOG_POST_BY_PATH } from '@/lib/queries'
...
interface BlogPostByPathData {
  route: {
    entity: DrupalBlogPost
  } | null
}

async function getBlogPost(path: string): Promise<DrupalBlogPost | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<BlogPostByPathData>({
      query: GET_BLOG_POST_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getBlogPost(path)

  if (!item) {
    return { title: 'Blog Post Not Found | Marketing Agency' }
  }

  return {
    title: `${item.title} | Marketing Agency`,
